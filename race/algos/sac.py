...
def is_rrd_like_method(rd_method):
    return "RRD" in rd_method or rd_method in {"VIB"}
...
class SAC(object):
    ...
    def train(self, replay_buffer, batch_size):
        ...
        if self.reward_model is not None and is_rarr_residual_method(self.args.rd_method):
            residual_reward = self.reward_model.forward(batch_s, batch_a, batch_s_)
            residual_eta = float(getattr(self.args, "rarr_residual_eta", getattr(self.args, "rarr_vib_eta", 1.0)))
            batch_r = batch_r + residual_eta * residual_reward.detach()
        ...
        if self.reward_model is not None and not self.args.direct_generate:
            if is_rarr_residual_method(self.args.rd_method):
                if self.args.rd_method == "RARR_RRD":
                    ...
                    traj_state, traj_action, traj_next_state, traj_reward, traj_dense_reward, traj_not_done, traj_episode_return, traj_episode_length = replay_buffer.sample_traj(
                        max(int(batch_size//np.mean(replay_buffer.episode_length)), 1),
                        length_priority=is_diaster_like_method(self.args.rd_method),
                    )
            ...
            if is_rarr_residual_method(self.args.rd_method):
                ...
                if isinstance(reward_model_loss, dict):
                    ...
                    train_info['reward_model_loss'] = reward_model_loss.get('rarr_vib_loss', 0.0)
                else:
