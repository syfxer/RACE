...
class PPO(object):
    ...
    def train(self,old_replay_buffer, replay_buffer, batch_size=256, total_steps=0):
        ...
        if self.reward_model is not None and is_rarr_residual_method(self.args.rd_method):
            residual_reward = self.reward_model.forward(s, a, s_)
            residual_eta = float(getattr(self.args, "rarr_residual_eta", getattr(self.args, "rarr_vib_eta", 1.0)))
            r = r + residual_eta * residual_reward.detach()
        elif self.reward_model is not None:
            r = self.reward_model.forward(s, a, s_).detach()
        ...
        if self.reward_model is not None and not self.args.direct_generate:
            if is_rarr_residual_method(self.args.rd_method):
                if self.args.rd_method == "RARR_RRD":
                    traj_num = max(int(batch_size//self.args.rrd_k), 1)
                    traj_state, traj_action, traj_next_state, traj_reward, traj_dense_reward, traj_not_done, traj_episode_return, traj_episode_length,_ = replay_buffer.sample_traj(traj_num)
                else:
                    traj_state, traj_action, traj_next_state, traj_reward, traj_dense_reward, traj_not_done, traj_episode_return, traj_episode_length,_ = replay_buffer.sample_traj(max(int(batch_size//np.mean(replay_buffer.episode_length)), 1), length_priority=is_diaster_like_method(self.args.rd_method))
                ...
                traj_state, traj_action, traj_next_state, traj_reward, traj_dense_reward, traj_not_done, traj_episode_return, traj_episode_length,_ = replay_buffer.sample_traj(batch_size//4, length_priority=True)
            else:
